...
import {ref} from "vue";
import {ModelMessage, ModelMessageQuote} from "../../model/ModelMessage";

export const useMessageStore = defineStore('message', () => {
        let data = ref<ModelMessage[]>([]);
        let currentChat = ref<ModelChat | null>(null);
        let currentQuote = ref<ModelMessageQuote | null>(null);
        ...
        let setQuote = (message: ModelMessage) => {
            if (!message.id || !message.fromName || !message.messageContent) return;
        ...
        let sendMessage = (content: string) => {
            ...
            if (currentQuote.value) {
                model.quote = {...currentQuote.value};
                clearQuote();
            }
        ...
        return {data, initData, sendMessage, currentChat, currentQuote, setQuote, clearQuote};
    },
    {
        persist: true,
    }
